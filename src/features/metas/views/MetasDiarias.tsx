...
export const MetasDiarias: React.FC = () => {
  ...
  const getContextoConfig = (contexto: string) => {
    const configs: Record<string, any> = {
      estudos: { icon: BookOpen, gradient: 'from-purple-500 to-indigo-600', iconColor: 'text-purple-600', bgIcon: 'bg-purple-100 dark:bg-purple-900/30', textColor: 'text-purple-600', bgLight: 'bg-purple-50 dark:bg-purple-900/20' },
      exercicios: { icon: Dumbbell, gradient: 'from-orange-500 to-red-600', iconColor: 'text-orange-600', bgIcon: 'bg-orange-100 dark:bg-orange-900/30', textColor: 'text-orange-600', bgLight: 'bg-orange-50 dark:bg-orange-900/20' },
      trabalho: { icon: Briefcase, gradient: 'from-blue-500 to-cyan-600', iconColor: 'text-blue-600', bgIcon: 'bg-blue-100 dark:bg-blue-900/30', textColor: 'text-blue-600', bgLight: 'bg-blue-50 dark:bg-blue-900/20' },
      financas: { icon: DollarSign, gradient: 'from-green-500 to-emerald-600', iconColor: 'text-emerald-600', bgIcon: 'bg-emerald-100 dark:bg-emerald-900/30', textColor: 'text-emerald-600', bgLight: 'bg-emerald-50 dark:bg-emerald-900/20' },
      saude: { icon: Heart, gradient: 'from-red-500 to-pink-600', iconColor: 'text-red-600', bgIcon: 'bg-red-100 dark:bg-red-900/30', textColor: 'text-red-600', bgLight: 'bg-red-50 dark:bg-red-900/20' },
      lazer: { icon: Smile, gradient: 'from-pink-500 to-rose-600', iconColor: 'text-pink-600', bgIcon: 'bg-pink-100 dark:bg-pink-900/30', textColor: 'text-pink-600', bgLight: 'bg-pink-50 dark:bg-pink-900/20' },
      outros: { icon: Target, gradient: 'from-gray-500 to-slate-600', iconColor: 'text-gray-600', bgIcon: 'bg-gray-100 dark:bg-gray-900/30', textColor: 'text-gray-600', bgLight: 'bg-gray-50 dark:bg-gray-900/20' }
    };
    // Fallback simples para contextos não mapeados
    const fallback = { icon: Target, gradient: 'from-gray-500 to-slate-600', iconColor: 'text-gray-600', bgIcon: 'bg-gray-100 dark:bg-gray-900/30', textColor: 'text-gray-600', bgLight: 'bg-gray-50 dark:bg-gray-900/20' };
  ...
  if (termoPesquisa.trim()) {
    ...
    metasDoDia = metasDoDia.filter(meta => 
    ...
    if (filtroImportancia !== 'todas') {
      metasDoDia = metasDoDia.filter(meta => meta.importancia === filtroImportancia);
    }
  } else {
    // Sem pesquisa: aplica filtros de status e importância normalmente
    if (filtroAtivo !== 'todos') {
      metasDoDia = metasDoDia.filter(meta => meta.status === filtroAtivo);
    }
  ...
  useEffect(() => {
    if (todasMetasDoDia.length > 0) {
      const todasConcluidas = todasMetasDoDia.every(meta => meta.status === 'concluida');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-slate-950 dark:via-gray-950 dark:to-slate-900 relative overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-6 sm:mb-8">
          <div className="flex-1">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-1">
              {isHoje ? "Metas de Hoje" : `Metas Diárias do dia ${tituloData}`}
            </h1>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 capitalize">
              {infoData}
            </p>
          </div>
          <Button className="bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white shadow-lg shadow-cyan-500/30 transition-all duration-300 h-10 sm:h-11 px-4 sm:px-6 w-full sm:w-auto" onClick={abrirModalCriar}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {resumo.concluidas + tarefasDoDia.filter(t => t.status === 'concluida').length}
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {resumo.em_andamento + tarefasDoDia.filter(t => t.status === 'andamento').length}
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {resumo.pendentes + tarefasDoDia.filter(t => t.status === 'pendente').length}
                </p>
              </div>
        ...
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 lg:gap-6 mb-4 sm:mb-6">
          <div className="flex-[1.5]">
            <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Pesquisar:</p>
            <Input 
              type="text"
          ...
          <div className="flex-1">
            <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Filtrar por importância:</p>
         ...
         <Card className="p-3 sm:p-4 mb-4 sm:mb-6 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-950/30 dark:to-cyan-950/30 backdrop-blur-sm border-l-4 border-l-blue-500">
          <div className="flex items-start gap-2 sm:gap-3">
            <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center flex-shrink-0">
      ...
      <ModalCriarMeta
        ...
        onConfirm={async (dados) => {
          ...
          await carregarMetas('diaria', dataFormatada, {});
        }}
        tipo="diaria"
