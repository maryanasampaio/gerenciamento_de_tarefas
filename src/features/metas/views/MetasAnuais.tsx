...
export const MetasAnuais: React.FC = () => {
  ...
  usePeriodicDeadlineCheck(todasMetasDoAno, 12);

  // Aplicar filtros client-side
  let metasDoAno = todasMetasDoAno;
  
  // Se tem pesquisa, aplica pesquisa em TODAS as metas (ignora filtro de status)
  if (termoPesquisa.trim()) {
    const termoLower = termoPesquisa.toLowerCase();
    metasDoAno = metasDoAno.filter(meta => 
    ...
    if (filtroImportancia !== 'todas') {
      metasDoAno = metasDoAno.filter(meta => meta.importancia === filtroImportancia);
    }
  } else {
    // Sem pesquisa: aplica filtros de status e importância normalmente
    if (filtroAtivo !== 'todos') {
      metasDoAno = metasDoAno.filter(meta => meta.status === filtroAtivo);
    }
  ...
  useEffect(() => {
    if (todasMetasDoAno.length > 0) {
      const todasConcluidas = todasMetasDoAno.every(meta => meta.status === 'concluida');
      const celebracaoKey = `celebracao_ano_${anoSelecionado}`;
  ...
  }, [todasMetasDoAno, anoSelecionado]);
  ...
  const tarefasDoAno = tarefas.filter(tarefa => {
    ...
    return tarefaAno === anoSelecionado;
  });

  // Ordenar tarefas por importância
  const tarefasOrdenadas = [...tarefasDoAno].sort((a, b) => {
  ...
  const itensCompletos = [
    ...metasDoAno.map(meta => ({ ...meta, tipo: 'meta' as const })),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-slate-950 dark:via-gray-950 dark:to-slate-900 relative overflow-hidden">
      {mostrarCelebracao && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <Card className="p-4 sm:p-6 md:p-8 max-w-lg mx-3 sm:mx-4 text-center bg-gradient-to-br from-emerald-50 to-cyan-50 dark:from-emerald-950 dark:to-cyan-950 border-4 border-emerald-400 dark:border-emerald-600 animate-bounce-in">
              <div className="inline-block p-3 sm:p-4 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500 mb-3 sm:mb-4">
              ...
              <p className="text-base sm:text-lg text-gray-700 dark:text-gray-300 mb-4 sm:mb-6">
                Parabéns! Você completou todas as {todasMetasDoAno.length} {todasMetasDoAno.length === 1 ? 'meta' : 'metas'} do ano.
              </p>
              <Button
                onClick={handleFecharCelebracao}
                className="bg-gradient-to-r from-emerald-500 to-cyan-600 hover:opacity-90 text-white px-4 sm:px-6 py-2 sm:py-3"
              >
                Continuar
              </Button>
            </Card>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-40">
        ...
        >
          <CalendarRange className="h-32 w-32" strokeWidth={1} />
        </div>
        <div 
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {resumo.concluidas + tarefasDoAno.filter(t => t.status === 'concluida').length}
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {resumo.em_andamento + tarefasDoAno.filter(t => t.status === 'andamento').length}
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">
                  {resumo.pendentes + tarefasDoAno.filter(t => t.status === 'pendente').length}
                </p>
              </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-gray-700 dark:text-gray-300">
                <strong>Crie uma meta, clique nela e defina pequenas tarefas para cumpri-la!</strong> 🎯 Metas organizam seu ano em objetivos maiores.
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-12 text-center bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm">
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-lg">
              Nenhuma meta ou tarefa para este ano
            </p>
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">
              Comece definindo suas metas para {anoSelecionado}
            </p>
          </div>
        </Card>
        ) : (
